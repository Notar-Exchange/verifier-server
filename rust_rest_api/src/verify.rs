...
pub async fn verify(proof: &str, notary_pubkey_str: &str) -> Result<String, String> {
    let proof: TlsProof = serde_json::from_str(proof)
        .map_err(|e| format!("Could not deserialize proof: {:?}", e))?;
    ...
    let (mut sent, mut recv) = substrings
        .verify(&header)
        .map_err(|e| format!("Could not verify substrings: {:?}", e))?;
    ...
    info!(
        "{}",
        String::from_utf8(sent.data().to_vec()).map_err(|e| format!(
    ...
    info!(
        "{}",
        String::from_utf8(recv.data().to_vec()).map_err(|e| format!(
    ...
    let result = VerifyResult {
        ...
        sent: String::from_utf8(sent.data().to_vec()).map_err(|e| {
            format!("Could not convert sent data to string: {:?}", e)
        })?,
        recv: String::from_utf8(recv.data().to_vec()).map_err(|e| {
            format!("Could not convert recv data to string: {:?}", e)
        })?,
    };
...
fn get_notary_pubkey(pubkey: &str) -> Result<p256::PublicKey, String> {
    p256::PublicKey::from_public_key_pem(pubkey)
        .map_err(|e| format!("Could not get notary pubkey: {:?}", e))
}